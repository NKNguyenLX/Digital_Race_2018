...
class LabelTool():
	def __init__(self, master):
		...
		self.currentclass = 0
		self.cla_can_temp = []
		self.classcandidate_filename = 'ts.names'
		...
		# self.svSourcePath.set(os.path.join(os.getcwd(),'../traffic-sign/0001'))
		self.svSourcePath.set('../traffic-sign/0001')
		...
		self.svDestinationPath.set("../traffic-sign/end-end-labels/")
	...
	def loadImage(self):
		...
		self.current_img_size = self.img.size
		self.current_img_path = imagepath
		size = self.img.size
		...
		if os.path.exists(self.labelfilename):
			with open(self.labelfilename) as f:
				for (i, line) in enumerate(f):
					...
					self.bboxList.append((label_id, x1, y1, x2, y2, tmp[5]))
					...
					self.listbox.insert(END, '%s : (%d, %d) -> (%d, %d)' %(tmp[5], tmp[1], tmp[2], tmp[3], tmp[4]))
					self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[color_index])
					#self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])
		print('Current image: ', self.imagename)
	...
	def saveImage(self):
		...
		copyfile(self.current_img_path, '../traffic-sign/images/'+self.current_img_path.split('/')[-1])
		# self.current_img.save(self.imageList[self.cur - 1])
	...
	def mouseClick(self, event):
		if self.STATE['click'] == 0:
			...
			self.bboxList.append((self.currentclass, x1*self.factor, y1*self.factor, x2*self.factor, y2*self.factor, self.currentLabelclass))
	...
	def setClass(self):
		self.currentclass = self.classcandidate.current()
		self.currentLabelclass = self.classcandidate.get()
		print('set label class to : %d %s' % (self.currentclass, self.currentLabelclass))
